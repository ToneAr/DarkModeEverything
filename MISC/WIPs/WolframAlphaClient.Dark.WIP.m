		...
		*)
		...
		Column[{
			...
			innerFrame[
				...
				Background -> RGBColor[1., 1., 0.92](*TONY: Dont know *),
	...
	Cell[
		BoxData[ToBoxes[
			innerFrame[
				...
				Background -> GrayLevel[(*0.96*)0]TONY: ama change this anyway,
				ImageSize -> Automatic,
	...
	Framed[
		Grid[{{
			Item[TextCell[Row[{
				...
				Background -> White(*TONY: Dont Know*),
				Frame -> 1,
				FrameStyle -> LightGray (*TONY: DK*)
			]}}],
		BaseStyle -> "DialogStyle",
		RoundingRadius -> 5,
		FrameStyle -> LightGray(*TONY: DK*),
		Background -> GrayLevel[0.965](*TONY: DK*),
		FrameMargins -> 5
	]
...
outerFrame[expr_, padding_] := 
	Framed[expr,
		RoundingRadius -> 8,
		FrameStyle -> LightGray(*TONY: Dont know what this changes yet*),
		Background -> Black(*RGBColor[0.0941176, 0.0941176, 0.0941176]*)(*TONY: Outer frame BG color*),
...
innerFrame[expr_, opts___] := 
	Framed[expr,
		opts,
		RoundingRadius -> 5,
		FrameStyle -> Gray(*TONY: Inner pods frame color*),
		Background -> White(*TONY: Dont know*),
		ImageSize -> Full,
		Alignment -> Top
	]
...
iTimeZoneToGMTString[offset_] := With[{n = N[offset]},
	If[
		...
		Switch[
			Sign[n],
			-1, StringJoin[$TimeZoneStringPrefix, StringTrim[ToString[n], "."]],
			0, $TimeZoneStringPrefix,
			1, StringJoin[$TimeZoneStringPrefix, "+", StringTrim[ToString[n], "."]],
			_, None
		],
		None
...
Block[{numberOfSubpods, subpods, podinfos, poddefinitions, podnotes, $subpodCounter = 0, subpodsAreSteps, steptypes},
	...
	dogEarPanel[
		innerFrame[
			...
			Background -> queryBlobBackground[Dynamic[q], Dynamic[newq], GrayLevel[0.965](*TONY: DK*)],
...
formatPodInfoUnits[{___, XMLElement["cell", _, {cell_}], ___}] :=
	podInfoOpener[
		"Units",
		Style[prepareSubpodCellContent[cell, Automatic, ResetAndUndeploy], Background -> White(*TONY: Not sure yet*)]
	]
...
queryBlobBackground[Dynamic[q_], Dynamic[newq_], color_:RGBColor[1., 0.975, 0.95]] := 
	Dynamic[
		FEPrivate`If[
			...
			color,
			GrayLevel[0.12(*1*)](*TONY: inner pod background*)
		]
	] /; StringQ[q]

queryBlobBackground[else___] := GrayLevel[0(*1*)](*TONY: DK*)
